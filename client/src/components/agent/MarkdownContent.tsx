...
function renderInline(text: string): React.ReactNode {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[2] !== undefined) {
      ...
      // *italic*
      parts.push(
        <em key={key++} className="italic text-text-secondary not-italic">
          {match[3]}
        </em>,
      )
    ...
    <div className={cn('flex flex-col gap-[7px]', className)}>
      {blocks.map((block, i) => {
        ...
        switch (block.type) {
          ...
          case 'h1':
            return (
              <p
                key={i}
                className={cn(
                  'text-[15px] font-bold text-text-primary leading-snug tracking-tight',
                  notFirst && 'mt-2',
                )}
              >
          ...
          case 'h3':
            return (
              <p
                key={i}
                className={cn(
                  'text-sm font-medium text-text-secondary leading-snug',
                  notFirst && 'mt-1',
                )}
              >
                {renderInline(block.text)}
          ...
          case 'bullet-list':
            return (
              <ul key={i} className="flex flex-col gap-[3px]">
                {block.items.map((item, j) => (
                  <li key={j} className="flex gap-2 text-sm text-text-primary leading-relaxed">
                    <span className="text-text-muted shrink-0 select-none mt-[1px]">•</span>
          ...
          case 'ordered-list':
            return (
              <ol key={i} className="flex flex-col gap-[3px]">
                {block.items.map((item, j) => (
                  <li key={j} className="flex gap-2 text-sm text-text-primary leading-relaxed">
                    <span className="font-mono text-[11px] text-text-muted shrink-0 select-none mt-[2px] min-w-[14px]">
                      {j + 1}.
                    </span>
                    <span>{renderInline(item)}</span>
                  </li>
          ...
          case 'code-block':
            return (
              <pre
                key={i}
                className="bg-surface-3 border border-border rounded-md px-3 py-2.5 overflow-x-auto"
              >
                <code className="font-mono text-[11.5px] text-text-primary leading-relaxed whitespace-pre">
                  {block.code}
                </code>
              </pre>
            )
          ...
          case 'blockquote':
            return (
              <blockquote
                key={i}
                className="border-l-2 border-border-hover pl-3 text-sm text-text-secondary italic"
              >
                {renderInline(block.text)}
              </blockquote>
            )
          ...
          case 'hr':
            return <hr key={i} className="border-t border-border my-0.5" />
