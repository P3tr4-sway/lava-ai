...
export function CompactDawStrip({ dawProps, className }: CompactDawStripProps) {
  ...
  return (
    <div
      className={cn(
        'border-t border-border bg-surface-1 overflow-y-auto',
        expanded ? 'h-[200px]' : 'h-[var(--editor-daw-collapsed-height)]',
        className,
      )}
    >
      ...
      <button
        ...
        aria-label={expanded ? 'Collapse DAW panel' : 'Expand DAW panel'}
      ...
      >
        ...
        <span
          ...
          className={cn('size-1.5 rounded-full', isPlaying ? 'bg-success' : 'bg-text-muted')}
        />
      </button>

      {/* Expanded DawPanel */}
      {expanded && (
        <div className="h-[calc(100%-var(--editor-daw-collapsed-height))]">
