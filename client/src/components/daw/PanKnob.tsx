...
export function PanKnob({ value, onChange }: PanKnobProps) {
  ...
  return (
    <div
      ...
      className="w-6 h-6 rounded-full bg-text-primary/15 border-2 border-text-primary/30 shrink-0 flex items-center justify-center cursor-ns-resize select-none"
      title={`Pan: ${label}`}
    >
      <div
        className="w-0.5 h-[10px] bg-text-primary/70 rounded-full"
