...
import type { BarlineType, Clef, Dynamic, RepeatMarker, ScoreDocument, ScoreMeasureMeta, ScoreNoteEvent, ScorePitch } from '@lava/shared'
...
export function useEditorCommandBridge(enabled = true): void {
  useEffect(() => {
    ...
    const handleDynamic = (event: Event) => {
      const value = (event as CustomEvent<{ value?: string }>).detail?.value
      if (!value) return
      const { track } = getTrackDocument()
      const { selectedNoteIds } = useEditorStore.getState()
    ...
    window.addEventListener('lava-dynamic', handleDynamic as EventListener)
    ...
    return () => {
      ...
      window.removeEventListener('lava-dynamic', handleDynamic as EventListener)
